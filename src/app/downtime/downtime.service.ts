...
export class DowntimeService {
  constructor(
    @InjectRepository(Downtime) private readonly downtimeRepository: Repository<Downtime>,
  ) {}

  public async create(downtime: DowntimeRequestCmd): Promise<Downtime> {
    try {
  ...
  public async findByCategoryForReport(
    category_id: number,
    params: DowntimeGetbylineShiftDateCmd,
  ): Promise<any> {
    let downtime: any;
    let rawQuery =
      'SELECT @no := @no + 1 as n,' +
  ...
  public async findByCategoryForAllReport(
    category_id: number,
    params: DowntimeGetbylineShiftDateCmd,
  ): Promise<any> {
    let downtime: any;
  ...
  public async findDowntimeEvent(line_id: number, params: OeeShiftDateShiftCmd): Promise<any> {
    ...
    try {
      data = await this.downtimeRepository.query(rawQuery, [params.date, params.shiftId, line_id]);
    } catch (error) {}
  ...
  public async findByLineDateShiftCategory(
    category_id: number,
    line_id: number,
    params: OeeShiftDateShiftCmd,
  ): Promise<any> {
    let downtime: Downtime[];
  ...
  public async findParetoDowntimeLoss(
    categoryId: number,
    params: AnalysisTimePeriodicCmd,
  ): Promise<any> {
    let data: any;
    let rawQuery =
      'SELECT b.reason as reason,' +
  ...
  public async getStatisticTimePeriodic(
    categoryId: number,
    params: AnalysisTimePeriodicCmd,
  ): Promise<any> {
    let data: any;
    let rawQuery =
      'SELECT COUNT(a.duration) AS total_accumulation,' +
